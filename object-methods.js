...
salesByWeek.forEach(allSales => {
    const names = allSales["sales_agent"]
    let p =" "
    const allCars = Object.entries(allSales["vehicle"])
    allCars.forEach(smaller =>{
        return p += `<p>${smaller[0]} : ${smaller[1]}<p>`
    })
    document.querySelector("#names").innerHTML += `<h1>${names["first_name"]} ${names["last_name"]}</h1>${p}<h2>Profits: $${allSales["gross_profit"]}</h2>`
})
